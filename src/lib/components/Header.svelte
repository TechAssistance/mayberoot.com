		...
		>
			<div class="flex items-center justify-between">
				...
				<button
					...
					class="block p-2 text-2xl text-indigo-100 md:hidden"
					on:click={() => (open = true)}
				>
					<IconMenu />
				</button>
			...
			<!-- Mobile Nav -->
			<ul
				class={`fixed inset-0 z-50 flex flex-col items-end gap-4 bg-sky-950 pr-4 pt-14 transition-transform duration-300 ease-in-out md:hidden ${open ? 'translate-x-0' : 'translate-x-[100%]'}`}
			>
				<li>
					<button
						aria-expanded={open}
						aria-label="Close Menu"
						class="fixed right-4 top-3 block p-2 text-2xl text-indigo-100 md:hidden"
						on:click={() => (open = false)}
					>
						<IconClose />
					</button>
